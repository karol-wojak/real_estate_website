      ...
      {projects.map((project) => (
        ...
        >
          ...
          <div className="w-full md:w-2/5 flex-shrink-0">
            {/* Using project.mainImage from the updated App.jsx data structure */}
            <img
              src={project.mainImage}
              alt={project.name}
              className="portfolio-image-aspect w-full object-cover rounded-md transition-transform duration-300 hover:scale-105"
              onError={(e) => { e.target.onerror = null; e.target.src = `/image_error_placeholder.png`; }} // Ensure placeholder path is correct or imported
            />
          </div>
          ...
          <div className="flex-grow text-left overflow-y-auto flex flex-col justify-center">
            ...
            {/* Displaying only the first sentence or a truncated description for the list view */}
            <p className="text-gray-800 md:text-lg mb-3">{project.description.split('.')[0]}.</p> {/* Display only first sentence */}
          ...
          <div className="w-full md:w-auto flex-shrink-0 md:self-center">
            ...
            <Link
              to={`/projects/${project.id}`} // Link to the new project detail page
              className="w-full md:w-auto px-6 py-2 rounded-full font-semibold shadow-md focus:outline-none transition-all duration-300
                         btn-gradient-dark-blue hover:btn-gradient-gold text-white text-center inline-block" // Added text-white & text-center for Link
            >
              Zobacz szczegóły
            </Link>
          </div>
